...
- `internal/api` — `NewRouter()` wires every route. It's the only package that imports all of `auth`, `tourdates`, `users`, and `representatives`. Both `cmd/server` and `internal/testutil` build the router through this one function, so tests exercise real production routing.
...
- `internal/tourdates`, `internal/users`, `internal/representatives` — one package per resource. Each owns its model, its `Handler{DB *pgxpool.Pool}`, and that handler's methods (`List`, `Get`, `Create`, `Patch`, `Delete` as applicable) — there's no shared "App" struct; each `Handler` gets the pool independently.
